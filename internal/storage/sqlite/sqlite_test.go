...
func TestPersistenceAndMigrationChecksums(t *testing.T) {
	...
	if err != nil || status.SchemaVersion != 1 || status.JournalMode != "wal" || status.EndpointCount != 1 {
...
func TestFutureSchemaRejected(t *testing.T) {
	...
	if _, err = s.db.Exec("INSERT INTO schema_migrations VALUES (2,'future','unknown')"); err != nil {
		t.Fatal(err)
	}
	_ = s.Close()
	if s, err = Open(t.Context(), path); !errors.Is(err, store.ErrSchema) {
