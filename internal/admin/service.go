...
func New(store UserStore, params security.PasswordParams) (*Service, error) {
	...
	return &Service{store: store, params: params, setupToken: token, sessions: map[[32]byte]Session{}, now: func() time.Time { return time.Now().UTC() }}, nil
}
...
func (s *Service) Setup(ctx context.Context, token, username, password string) (auth.User, error) {
	...
	defer s.mu.Unlock()
	needed, err := s.SetupRequired(ctx)
	if err != nil || !needed || subtle.ConstantTimeCompare([]byte(token), []byte(s.setupToken)) != 1 || !validUsername(username) {
