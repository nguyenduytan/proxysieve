...
ProxySieve accepts manual parser/import previews and has a safe HTTP(S) source-fetch
primitive for future source scheduling/API integration. A refresh first validates an
...
invalid and duplicate records without persisting endpoints. The source scheduler,
mapping configuration, source CRUD/API, and persistence of source status remain
future milestones. Existing endpoint records are never deleted merely because a
refresh fails.
