...
| Operations | SQLite migrations, first-run admin setup, Argon2id password hashing, session-bound CSRF, RBAC, audited user/proxy/source/pool/policy/client/API-key APIs, authenticated bounded traffic and operational-event SSE, signed SSRF-safe webhook alerts, cache stats/full/exact-host purge API and CLI, canonical policy simulation, atomic proxy-source refresh API/scheduler, WAL-consistent local backup/restore, versioned safe config import/export and published OpenAPI contract | Shadow comparison and external extensions |
