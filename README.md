...
| Operations | SQLite migrations, first-run admin setup, Argon2id password hashing, session-bound CSRF, RBAC, atomic audited proxy-source refresh and client/API-key lifecycle APIs, published OpenAPI contract | Source refresh scheduling/UI, API-key list/revocation UI, SSE, backup/restore |
